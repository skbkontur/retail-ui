...
describe('ComboBoxRenderer', () => {
  ...
  it.skip('does not try to recover if closed by Enter without editing', () => {
    ...
    const wrapper = mount(
      <ComboBoxRenderer value="foo" source={source} onChange={onChange} recover={recover} alkoValueToText={x => x} />
    );
  ...
  it.skip('tries to recover if opened by entering a char', () => {
    ...
    wrapper.find('[tabIndex]').simulate('keypress', { charCode: 'a'.charCodeAt(0) });
