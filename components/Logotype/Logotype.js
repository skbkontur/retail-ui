...
const createPngCloud = backgroundColor => (
  <div style={{ backgroundColor }} className={styles.cloud}>
    <div style={{ backgroundImage: `url('${cloudImage}')` }} className={styles.inner} />
  </div>
);
...
// eslint-disable-next-line react/no-multi-comp
const Logotype = ({ color, textColor, component: Component, suffix, href }: Props) => (
  <Component href={href} tabIndex="-1" className={styles.root}>
    <span style={{ color: textColor }}>к</span>
    <span style={{ color }}>{hasSVGSupport() ? createCloud(color) : createPngCloud(color)}</span>
