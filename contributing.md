...
Скриншотные тесты пишут для проверки верстки и отдельной функциональности в различных браузерах (Chrome, Firefox, IE11). В проекте они построены на основе [Creevey](https://github.com/wKich/creevey) и [Storybook](https://storybook.js.org/). Для удобства поддержки статичные и интерактивные тесты предлагается разделить по отдельным стори. Creevey тесты необходимо выделить в `__stories__/[ComponentName].creevey.stories.tsx`. Под статичными тестами подразумеваются тесты, в которых рендерятся элементы и не производятся никакие действия над ними. Под интерактивными тестами подразумеваются тесты, где производятся разные пользовательские манипуляции.
