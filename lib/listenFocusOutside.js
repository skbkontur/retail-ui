...
function addHandleEvent() {
   ...
   * trying to catch blur events, and checking explicitOriginalTarget
   */
...
function handleNativeFocus(event: UIEvent) {
  const target: HTMLElement = isFF
    // http://stackoverflow.com/a/121708/1555589
    ? (event: any).explicitOriginalTarget
    : (event.target || event.srcElement: any);
