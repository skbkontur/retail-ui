 ...
 */
function findRenderContainer(node: HTMLElement, rootNode: HTMLElement, container?: HTMLElement): ?HTMLElement {
  ...
  if (newContainerId) {
    const nextNode = document.querySelector(`noscript[data-render-container-id="${newContainerId}"]`);
...
function listen(elements: Array<HTMLElement> | (() => Array<HTMLElement>), callback: (event: Event) => void) {
  const handler = {
    elements,
    callback
  };
