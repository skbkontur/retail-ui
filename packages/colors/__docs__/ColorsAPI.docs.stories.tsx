...
import type { TokensBase } from '../lib/types/tokens-base.js';
  ...
  const colorGroups = Object.entries(KonturColors).reduce(
    (acc: Record<string, Record<string, string>>, [colorKey, colorValue]) => {
      const firstWord =
        ...
        ['greenMint', 'blueDark'].find((color) => colorKey.match(color)) ||
        // oxlint-disable-next-line typescript/no-non-null-assertion
        colorKey.match(/^[a-z]+/)![0]!;
    ...
    return tokensToDisplay
      .map((token) => {
        ...
        if (isSimilarSearch && parsedFilterColor) {
          ...
          if (tokenColor) {
            const distance = differenceEuclidean('rgb')(
              parsedFilterColor as unknown as string,
              tokenColor as unknown as string,
            );
