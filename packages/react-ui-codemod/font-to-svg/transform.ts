// @ts-nocheck — AST jscodeshift, перенос с существующего кода
...
function patchIconToNamedIcon({
  ...
  if (isSimpleIcon(nameAttr)) {
    ...
    if (path.parent.parent.node.type === 'JSXExpressionContainer') {
      ...
      const iconNodeIndex = path.parent.parent.node.children.findIndex((child: any) => child === path.parent.node);
...
function patchIconToNamespace({ j, path, nameAttr, nameAttrIndex, iconName }: any) {
  if (!nameAttr) {
    return;
  }
...
function patchIconPropToNamedIcon({ j, iconAttr, iconName, consequentIconName, alternateIconName }: any) {
...
function patchIconPropToSimpleIcon({ j, iconAttr, iconName, expressionIconValue }: any) {
  if (isSimpleIcon(iconAttr)) {
    iconAttr.value = j.jsxExpressionContainer(
      j.jsxElement(
...
function isSimpleIcon(attr: any) {
  return (
...
function isTernaryIcon(attr: any) {
  return (
    attr.value &&
    attr.value.expression &&
...
export default function transform(file: FileInfo, api: API) {
  ...
  root
    .find(j.JSXOpeningElement, (node: any) => componentNames.includes(node.name.name))
    .forEach((path: any) => {
      if (path.node.name.name === 'Icon') {
        const nameAttrIndex = path.node.attributes.findIndex(
          (attr: { name: { name: string } } | any) => attr.name && attr.name.name === 'name',
        );
        ...
        const iconAttrIndex = path.node.attributes.findIndex(
          (attr: { name: { name: string } } | any) => attr.name && attr.name.name === 'icon',
        );
        const iconAttr = path.node.attributes[iconAttrIndex];
        const component: any = { j, path, iconAttr };
  ...
  iconComponents.forEach((iconComponent: any) => {
  ...
  componentsWithIconProp.forEach((component: any) => {
    if (preserveIconImport) {
  ...
  imports.replaceWith((path: any) => {
    const specifiers = path.node.specifiers;
    const filteredSpecifiers = specifiers.filter((spec: { local: { name: string } }) => spec.local.name !== 'Icon');

    if (specifiers.length === filteredSpecifiers.length) {
      return path.node;
    }

    if (preserveIconImport && filteredSpecifiers.length === 0) {
      return null;
    }
  ...
  if (!preserveIconImport) {
    if (!iconImportsInserted) {
      imports.at(-1).insertAfter(iconImports);
    }
  }
