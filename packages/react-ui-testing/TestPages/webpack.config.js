  ...
  return {
    ...
    plugins: [
      new webpack.DefinePlugin({
        ...
        "process.env.hasKebab": JSON.stringify(
          semver.satisfies(retailUIVersion, ">=0.9.0")
        ),
        "process.env.hasPaging": JSON.stringify(
          semver.satisfies(retailUIVersion, ">=0.9.0")
        ),
        "process.env.hasSidePage": JSON.stringify(
          semver.satisfies(retailUIVersion, ">=0.11.0")
        ),
        "process.env.newCombobox": JSON.stringify(
          semver.satisfies(retailUIVersion, ">=0.7.0")
        ),
        "process.env.retailUIVersion": JSON.stringify(retailUIVersion),
        "process.env.baseUrl": JSON.stringify(
          `/${reactVersion}/${retailUIVersion}`
        )
      }),
...
module.exports = versions.map(version =>
  createConfig(version["react"], version["retail-ui"])
);
