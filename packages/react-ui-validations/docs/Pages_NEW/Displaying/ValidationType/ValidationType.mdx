...
Обратите внимание: у `TimePicker` событие `onValueChange` срабатывает только при коммите значения (потеря фокуса, выбор элемента из списка, нативный пикер).
Поэтому мгновенная валидация, вычисляемая по `value`, обновится только после коммита.
Для валидации по мере ввода используйте событие `onInputValueChange`.
...
Такую валидацию можно сделать <Link onClick={linkTo('Validator/Independent')}>независимой</Link>.
