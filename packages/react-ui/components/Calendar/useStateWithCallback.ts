...
export function useStateWithCallback<T = undefined>(init?: T): [T | undefined, SetStateAction<T | undefined>];
export function useStateWithCallback<T>(init: T): [T, SetStateAction<T>] {
  const [state, setState] = useState<T>(init);
  const cbRef = useRef<OnUpdateCallback<T>>();

  const setStateWithCallback: SetStateAction<T> = (newState, callback?): void => {
