...
export const IndeterminateWithSiblings: Story = () => {
  const [checkedSiblings, setCheckedSiblings] = useState<number[]>([]);
  const siblingCheckboxes = [1, 2];

  let parentCheckboxRef: Nullable<Checkbox>;
...
IndeterminateWithSiblings.storyName = 'indeterminate with siblings';
IndeterminateWithSiblings.parameters = {
  creevey: {
    tests: {
      async idle() {
        await this.expect(await this.takeScreenshot()).to.matchImage('idle');
      },
      async firstSiblingChecked() {
        const labels = await this.browser.findElements({ css: 'label' });
        ...
        await this.expect(await this.takeScreenshot()).to.matchImage('firstSiblingChecked');
      },
      async secondSiblingChecked() {
        const labels = await this.browser.findElements({ css: 'label' });
        ...
        await this.expect(await this.takeScreenshot()).to.matchImage('secondSiblingChecked');
      },
      async secondSiblingUnchecked() {
        const labels = await this.browser.findElements({ css: 'label' });
        ...
        await this.expect(await this.takeScreenshot()).to.matchImage('secondSiblingUnchecked');
      },
      async firstSiblingUnchecked() {
        const labels = await this.browser.findElements({ css: 'label' });
        ...
        await this.expect(await this.takeScreenshot()).to.matchImage('firstSiblingUnchecked');
      },
    },
  },
};
