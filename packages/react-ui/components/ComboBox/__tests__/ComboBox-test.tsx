...
describe('ComboBox', () => {
  ...
  it('selects the first item when search found multiple items', async () => {
    ...
    await userEvent.click(root);
    await userEvent.type(root, 'Item ');
    await delay(100); // wait for load and render search list
    await userEvent.keyboard('{enter}');
    await waitFor(() => expect(onValueChange).toHaveBeenCalledWith('Item 1'));
  });
  ...
  describe('update input text when value changes if there was no editing', () => {
    const getItems = vi.fn((searchQuery: string) =>
