...
import { Button, Group, DateOrder, DateSeparator, Gapped, Tooltip, LocaleContext, Select, LangCodes } from '@skbkontur/react-ui';
...
Имеется статический метод `DatePicker.validate`, который проверяет, что введенная дата корректна
...
Docs: https://data.gov.ru/api-portala-otkrytyh-dannyh-rf-polnoe-rukovodstvo
...
[//]: # (```typescript static)
[//]: # (import {DateOrder, DateSeparator} from "@skbkontur/react-ui";)
[//]: # ()
