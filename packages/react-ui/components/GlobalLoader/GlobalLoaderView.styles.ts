...
export const getAnimations = (emotion: Emotion) => {
  const moveToRightAnimation = emotion.keyframes`
  0% {
    left: 0;
    width: 100%;
  }
...
`;
  const spinnerAnimation = emotion.keyframes`
  0% {
...
`;
  const linearProgressAnimation = emotion.keyframes`
  from { width: 0; }
  to { width: 80% }
`;
  const slowProgressAnimation = emotion.keyframes`
  from { width: 80%; }
  to { width: 90% }
`;
