...
export interface HintProps extends CommonProps {
  ...
  /** Переводит отображение подсказки в _"ручной режим"_. В _"ручном режиме"_ подсказку можно активировать только задав значение пропу `opened`. */
  manual?: boolean;
  ...
  /** Если `true` - подсказка будет открыта. _Примечание_: работает только при `manual=true`.*/
  opened?: boolean;
  /** Расположение подсказки относительно текста. */
  pos?: 'top' | 'right' | 'bottom' | 'left' | PopupPositionsType;
  /** Текст подсказки. */
  text: React.ReactNode;
  /** Список позиций, которые хинт будет занимать. Если положение хинт в определенной позиции будет выходить за край экрана, то будет выбрана следующая позиция. Обязательно должен включать позицию указанную в `pos`.Для применения этого пропа необходимо включить фиче-флаг hintAddDynamicPositioning. */
  allowedPositions?: PopupPositionsType[];
  /** Отключает анимации. */
  disableAnimations?: boolean;
  /** Явно указывает, что вложенные элементы должны быть обёрнуты в `<span/>`. <br/> Используется для корректного позиционирования хинта при двух и более вложенных элементах. _Примечание_: при **двух и более** вложенных элементах обёртка будет добавлена автоматически.*/
  useWrapper?: boolean;
}
