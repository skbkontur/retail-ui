      ...
      <Modal width={width} onClose={this.close.bind(this, name)}>
        ...
        <Modal.Body>
          {name === 'firstModalOpened' && (
            <Button data-tid="open-second" onClick={() => this.setState({ secondModalOpened: true })}>
              Open second modal
            </Button>
          )}
        </Modal.Body>
  ...
  public render() {
    ...
    return (
      <div>
        ...
        <Button data-tid="open-first" onClick={() => this.setState({ firstModalOpened: true })}>
          Open first modal
        </Button>
      </div>
    );
      ...
      <div style={{ width: '300px' }}>
        {this.state.opened && (
          <Modal onClose={this.close}>
            ...
            <Modal.Footer panel={this.state.panel}>
              <Button data-tid="open-modal" onClick={this.close}>
                Close
              </Button>
            </Modal.Footer>
          </Modal>
    ...
    tests: {
      async ['open first modal']() {
        await this.browser
          ...
          .click(this.browser.findElement({ css: '[data-tid="open-first"]' }))
          .perform();
        await delay(200);
      ...
      async ['open second modal']() {
        ...
        await this.browser
          .actions({ bridge: true })
          .click(this.browser.findElement({ css: '[data-tid="open-second"]' }))
          .perform();
        await delay(100);
...
export const MobileModal: Story = () => {
  ...
  const render = (
    <div>
      <Button data-tid="open-modal" onClick={() => setOpen(true)}>
        Open modal
      </Button>
      {isOpen && modal}
    </div>
