...
kind("Sticky", () => {
  story("WideContainer", () => {
    test("fixed", async function () {
      await this.browser.executeScript(function () {
        const stickyContent = window.document.querySelector(
          '[data-tid="stickyContent"]'
        ) as HTMLElement;
        const nonStickyText = window.document.querySelector(
          '[data-tid="nonStickyText"]'
        ) as HTMLElement;
  ...
  story("Top", ({ setStoryParameters }) => {
    setStoryParameters({
      skip: {
        "flacky stopped position": {
          ...
          tests: "stoped",
          reason: "flacky stopped position",
        },
      },
    });
    ...
    test("fixed", async function () {
      await this.browser.executeScript(function () {
        const stickyStop = window.document.querySelector(
          '[data-tid="stickyStop"]'
        ) as HTMLElement;
        const scrollOffset =
          stickyStop.getBoundingClientRect().top - window.innerHeight / 2;
    ...
    test("stoped", async function () {
      ...
      await this.browser.executeScript(function () {
        ...
        const scrollOffset =
          pageYOffset - stickyContent.getBoundingClientRect().height / 2;
  ...
  story("Bottom", ({ setStoryParameters }) => {
    ...
    test("fixed", async function () {
      await this.browser.executeScript(function () {
        const sticky = window.document.querySelector(
          '[data-comp-name~="Sticky"]'
        ) as HTMLElement;
        const scrollOffset =
          sticky.getBoundingClientRect().top - window.innerHeight;
