...
/** Файл весь построен на энзиме. Весь законментил, оставив один пустой тест:
 * Skipped Props Forwarding
 * Чтобы остальные скипнкутые тесты не вызвали ошибку "No matching tests found"
 * */
...
describe('Props Forwarding', () => {
  ...
  it('Skipped Props Forwarding', () => {
    React; // чтобы тест собирался без ошибок
  });

  describe('Common Props', () => {
    // const getTestDOMNode = (compName: string, wrapper: any) => {
    //   switch (compName) {
    //     case 'Hint':
    //     case 'Tooltip':
    ...
    //   }
    // };
    // it.each<[string, any]>(PUBLIC_COMPONENTS.map((name) => [name, createWrapper(name)]))(
    //   '%s',
  ...
  describe('"inputMode" prop', () => {
    ...
    it.skip.each<keyof typeof ReactUI>([
      'Input',
      'FxInput',
      'CurrencyInput',
      'PasswordInput',
